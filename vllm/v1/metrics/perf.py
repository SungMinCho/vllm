...
class PerfMetricsProm:
    ...
    def observe(self, perf_stats: PerfStats, engine_idx: int = 0):
        if not (
            perf_stats.num_flops
            or perf_stats.num_read_bytes
            or perf_stats.num_write_bytes
        ):
            return
        self.counter_flops[engine_idx].inc(perf_stats.num_flops)
        self.counter_read_bytes[engine_idx].inc(perf_stats.num_read_bytes)
        self.counter_write_bytes[engine_idx].inc(perf_stats.num_write_bytes)
